...
public class InspectorAccessInterceptor implements HandlerInterceptor {
    //网格员Inspector访问拦截器
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        try {
            ...
            if (!redisUtils.hasKey(token)) {
                logger.info("网格员token过期，请重新登录");
                //token失效 拦截请求
                throw new RuntimeException();
            } else {
                //刷新token有效期
                redisUtils.expire(token, 1, TimeUnit.HOURS);
                if (((String)redisUtils.get(token)).startsWith(RoleConstants.INSPECTOR) || ((String)redisUtils.get(token)).startsWith(RoleConstants.ADMIN)) {
                    //如果value以inspector或admin开头则放行访问
                    return true;
                } else {
                    logger.info("网格员权限不足");
                    //抛异常拦截请求
